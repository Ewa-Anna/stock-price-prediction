...
def predict_stock(request):
    if request.method == 'POST':
        ...
        if historical_data.empty:
            return render(request, 'predictor/main.html', {'error': 'No data found for this ticker.'})

        dates = historical_data['Date'].dt.strftime('%Y-%m-%d').tolist()
        prices = historical_data['Close']

        model = train_model(historical_data)
        future_date = np.array([[pd.Timestamp.now().timestamp() + 86400]]) # The next day: 86400 is 24h (24x60x60)
